...
- [Download LiquorBee Updater 1.0.1](LiquorBee-Updater-1.0.1.apk?raw=true)
...
This folder's `version.txt` is the updater's own build number (`2`). The POS release metadata remains at `LiquorBeePOS/version.txt` (`20260910`, matching POS 1.2.1). The updater never uses its own build number to compare POS versions.
